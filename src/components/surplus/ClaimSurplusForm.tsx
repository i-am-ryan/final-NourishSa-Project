// src/components/surplus/ClaimSurplusForm.tsx
...
export default function ClaimSurplusForm({ onCancel }: ClaimSurplusFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2) insert a claim record
      const { error: dbErr } = await supabase
        .from('food_claims')
        .insert({
          user_id:       user.id,
          claimant_type: formData.claimantType,
          name:          formData.name,
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        >
          ...
          <Select
            ...
            onValueChange={(value) =>
              setFormData({ ...formData, claimantType: value })
            }
          >
        ...
        <AnimatePresence>
          {formData.claimantType && (
            ...
            >
              <label className="block text-sm font-semibold text-gray-700 mb-3">
                {formData.claimantType === 'Households & Individuals'
                  ? 'Your Name'
                  : 'Organization Name'}
              </label>
              <Input
                  ...
                  <SelectContent className="rounded-xl shadow-lg border-0 bg-white/95 backdrop-blur-sm">
                    {Object.keys(locationData).map((city) => (
                      <SelectItem
                        key={city}
                        value={city}
                        className="rounded-lg my-1 hover:bg-gray-50 font-medium"
                      >
                        {city}
                      </SelectItem>
                    ))}
                  ...
                  >
                    ...
                    >
                      ...
                      <SelectContent className="rounded-xl shadow-lg border-0 bg-white/95 backdrop-blur-sm max-h-60">
                        {locationData[
                          formData.city as keyof typeof locationData
                        ]?.map((suburb) => (
                          <SelectItem
                            key={suburb}
                            value={suburb}
